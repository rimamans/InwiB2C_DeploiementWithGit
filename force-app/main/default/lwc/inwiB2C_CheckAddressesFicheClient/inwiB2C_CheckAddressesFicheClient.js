...
import template from './inwiB2C_CheckAddressesFicheClient.html';

import { getNamespaceDotNotation } from 'vlocity_cmt/omniscriptInternalUtils';
import { OmniscriptActionCommonUtil } from 'vlocity_cmt/omniscriptActionUtils';


export default class inwiB2C_CheckAddressesFicheClient extends OmniscriptBaseMixin(NavigationMixin(LightningElement)) {
    ...
    //CH-Y Ticket taskForce2025 B-17806 Begin
    //@api v;
        _v;
    @api 
    get v() {
    ...
    //CH-Y Ticket taskForce2025 B-17806 End
    ...
    //CH-Y Ticket taskForce2025 B-17806 Begin
    // Retourne true si le champ Ville est vide (pour désactiver le bouton)
    get isVilleValid() {
        return !this.v || this.v === 'null' || this.v.trim() === '';
    }
    
    // Méthode appelée au clic sur le bouton Suivant
    async handleNextStep() {
        if (!this.v || this.v === 'null' || this.v.trim() === '') {
            this.dispatchEvent(new CustomEvent('showtoast', {
                detail: {
                    title: 'Erreur',
                    message: 'Veuillez renseigner le champ Ville.',
                    variant: 'error'
                }
            }));
            return;
