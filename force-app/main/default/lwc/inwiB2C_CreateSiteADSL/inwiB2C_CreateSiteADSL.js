...
import template from "./inwiB2C_CreateSiteADSL.html";
...
export default class inwiB2C_CreateSiteADSL extends OmniscriptBaseMixin(NavigationMixin(LightningElement))
{
  @api message;
 ...
 // varclass = "slds-col slds-size_1-of-2 elementMargin2";
  ...
  createAccount() {
    ...
    input = '{"region": "' + this.region + '","ville": "' + this.villeid + '","parentId": "' + this.accountid + '","codepostale":"' + this.codepostal + '","complement":"' + this.complementaddress + '","quartier":"' + this.quartier + '","precisionquartier":"' + this.pquartier + '","nomsite":"' + this.nomSite + '","nomIAM":"' + this.nomIAM + '","prenomIAM":"' + this.prenomIAM + '","statutDegroupage":"' + this.valueStatus + '"}';//,"numND":"'+this.numND+'"
      ...
      .then(response => {
        ...
        if (!response.error) {
          if (response.result.IPResult.accountid != null) {
            ...
            this.accountidSite = response.result.IPResult.accountid;
            //  console.log('record: '+this.accountidSite);
  ...
  InfoDegroupage() {
    ...
    if (this.previous == false && this.ProvinceVille != null && this.Voie != null && this.QuartierIAM != null && this.CommuneV != null &&  this.ProvinceVille != '' && this.Voie !='' && this.QuartierIAM != '' && this.CommuneV != '') {
      ...
      console.log('in infoDeggroupage Suivant');
...
*/
  ...
  Synchronise() {
    ...
    if (this.requalification == true || this.valueStatus == 'Active') {//update (status deg active ou dans le cas de qualification de l'adresse)
      this.updateND();
      console.log('in if requalification egale true');
    } else {
      /* chb ano B-15870 07/05/2024 begin*/
      //this.createND();
      //this.previous = false;
  ...
  this.omniSaveState(traceServiceInfoAPI,true);
   /* chb ano B-19085 24/09/2024 end */
    ...
    if (typeof this.porte === 'undefined' || this.porte === null) {
      this.porte = '';
    } if (typeof this.etage === 'undefined'|| this.etage === null) {
      this.etage = '';
    } if (typeof this.batiment === 'undefined'|| this.batiment === null) {
      this.batiment = '';
    } if (typeof this.escalier === 'undefined'|| this.escalier === null) {
      this.escalier = '';
    }
    let adress;
    ...
    if (typeof this.Numerovoie === 'undefined'|| this.Numerovoie === null) {
      this.Numerovoie = '';
      this.labelNumVoie = '';
      adress = this.porte + '|' + this.etage + '|' + this.escalier + '|' + this.batiment + '|' + '|' + this.Voie + ';' + this.labelVoie + '|' + this.QuartierIAM + ';' + this.labelQuartier + '|' + this.CommuneV + ';' + this.labelCommune + '|' + this.ProvinceVille + ';' + this.labelProvince;

    } else {
      adress = this.porte + '|' + this.etage + '|' + this.escalier + '|' + this.batiment + '|' + this.Numerovoie + ';' + this.labelNumVoie + '|' + this.Voie + ';' + this.labelVoie + '|' + this.QuartierIAM + ';' + this.labelQuartier + '|' + this.CommuneV + ';' + this.labelCommune + '|' + this.ProvinceVille + ';' + this.labelProvince;
      ...
      .then(response => {
        ...
        if (response.error == false) {
          ...
          if (response.result) {
            ...
            this.disableAddInwiAfterModif=true;
            this.Step1 = true;
  ...
  handleKeyDownVilleSupport(event) {
    ...
    if (!regex.test(key) && key !== 'Backspace' && key !== 'ArrowLeft' && key !== 'ArrowRight' && key !== 'Delete') {
      ...
      // Regex pattern to detect only single and double quotes
      const regex = /^['"]$/;
  ...
  callAPIdemandeIAM() {
    ...
    /*if (!pattern.test(this.AdClient)) {
        this.showToast('Erreur', 'Adresse client invalide. Veuillez entrer une adresse correcte.', 'error');
        return;
    }*/
...
//chb ano B-14518 27/02/2024 end */
        ...
        if (!response.error) {
          if (response.result.IPResult && response.result.IPResult.characteristic) {
            ...
            this.__dataEliInterne.forEach(currentItem => {
              ...
              /* chb B-20359 24/09/2024 begin */
              this.technoPorte = currentItem.technoPorte.replace('&', '');
              /* chb B-20359 24/09/2024 end */
          ...
          else {/*chb 07/05/2024 ANO B-15870 begin*/
  ...
  disableAddInwiAfterModif=false;
