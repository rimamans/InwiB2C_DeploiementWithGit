...
export default class inwib2c_EligibilitTyFTTHTpeAcessIAMForMigration extends OmniscriptBaseMixin(
 ...
 async CheckEligiblite() {
  ...
  if (this.ND !== '' && this.ND != null) {
    ...
    input = `{"operateur":"iam", "ND":"${this.ND}"}`;
  } else if (this.Login !== '' && this.Login != null) {
    input = `{"operateur":"iam", "ND":"${this.Login}"}`;
  }
  ...
  try {
    ...
    if (!response.error) {
      ...
      this.isNDLoginEligible = (status === "OK" && oiOperator === "IAM"); 
