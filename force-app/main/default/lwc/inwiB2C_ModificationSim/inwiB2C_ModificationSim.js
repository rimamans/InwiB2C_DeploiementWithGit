...
export default class InwiB2C_ModificationSim extends OmniscriptBaseMixin(LightningElement) {
    __item;
    __iccAttributeCategoriesId;
    __iccProductAttributesId;
    __firstGet = true;
    @track __oldICC;
    @track __subscriptionICC;
    @track __assetICC;
    @track __newICC;
    ...
    connectedCallback() {
        ...
        if (this.__firstGet) {
            this.__oldICC = this.iccAttributeValue;
            this.__firstGet = false;
            this.iccAttributeValue = null;
            this.getExistingIcc();
        }
        if (this.item.action.includes("Change")) {
            //this.libelleICC = "Nouveau ICC :";
            //this.Changed=true;
    ...
    get Changed() {
        ...
        return this.__oldICC !== this.__subscriptionICC;

    }

    get isAssetIccDifIccSub() {

        return this.__assetICC !== this.__subscriptionICC;
    ...
    setReturnedAttributeValue(puk, imsi, ki, mdn, profile,ocsId, codeArticle, typeSim) {
        console.log("====================in setReturnedAttributeValue======================");
        let localItem = JSON.parse(JSON.stringify(this.__item));

        if (this.__iccAttributeCategoriesId >= 0 && this.__iccProductAttributesId >= 0) {
            localItem.attributeCategories.records[this.__iccAttributeCategoriesId].productAttributes.records[this.__iccProductAttributesId].userValues = this.__iccValue;
        }
        if (this.__pukAttributeCategoriesId >= 0 && this.__pukProductAttributesId >= 0) {
    ...
    handleKeyUp(event) {
        ...
        if (isEnterKey) {
            // Search B.E. service
            console.log('in isenterkey');
            var iccInput = this.template.querySelector(".iccInput");
            var value = iccInput.value;
            // is input valid text?
            if (value.length != 18 || !value.match(/^[0-9]+$/)) {
                iccInput.setCustomValidity("L'ICCID saisi est incorrect. il doit être sur 18 caractères et ne doit comporter que des chiffre.");
            } else {
            ...
            // mockup
            //this.setReturnedAttributeValue ('puk', 'imsi', 'ki', '212000000000', 'profile');
    ...
    searchICC() {
        ...
        let input = '{"handset": [{"code":"' + this.modeleCarteAttributeValue + '","serialNumber": "' + this.__iccValue + '","typeSim":"","type": "ICCID","technology": "GSM","offerType": "FM","orderType": "SMO",';
        input += '"vendor": "CPD01","distributorCode": "CPD01","region": "PGSM","quantity": "1"}],"operation": "RS", "orderId":"' + this.item.OrderId.value +'","username": "Djamel"}';
        ...
        const params = {
            ...
            sMethodName: 'Inwi_InwiB2C_LockArticleSAVSIM',
            options: '{}',
        };
                ...
                if (response.error == false) {
                    ...
                    if (response.result && response.result.IPResult && response.result.IPResult.CSOperationResultInfo) {
                        if (response.result.IPResult.CSOperationResultInfo.isOK && response.result.IPResult.CSOperationResultInfo.isOK == "1") {
                            ...
                            //this.setReturnedAttributeValue(puk, imsi, ki, mdn, profile, codeArticle);
                            // this.setReturnedAttributeValue (puk, imsi, ki, mdn, profile);
                        ...
                        // Meryem Yahya SF24-042 B-30509 
                            ...
                            this.dispatchEvent(
                                new ShowToastEvent({
                                    title: 'Erreur',
                                    message: 'Cette carte SIM n\'est pas disponible.',
                                    variant: 'error'
                                }),
                            );
    ...
    getExistingIcc() {
        ...
        console.log('in getExistingIcc');

        let input = '{"OrderId": "'+ this.item.OrderId.value+'"}';
        ...
        const params = {
            input: input,
            sClassName: `${this._ns}IntegrationProcedureService`,
            sMethodName: 'inwib2c_GetICCAssetSubFromOrder',
            options: '{}',
        };
            ...
            .then(response => {
                ...
                if (response.error == false) {
                    ...
                    if (response.result && response.result.IPResult ) {
                        this.__subscriptionICC = response.result.IPResult.ICC;
                        this.__assetICC = response.result.IPResult.ICCAsset;
                    }
                } else {
                    console.log('Erreur lors de la récupération de ICC Ancien');
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Erreur',
                            message: 'Erreur lors de la récupération des informations de ICC Ancien',
                            variant: 'error'
                        }),
                    );
                }
