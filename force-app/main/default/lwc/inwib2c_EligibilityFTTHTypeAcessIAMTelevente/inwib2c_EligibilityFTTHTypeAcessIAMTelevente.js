...
export default class Inwib2c_EligibilityFTTHTypeAcessIAMTelevente extends OmniscriptBaseMixin(
  ...
  connectedCallback() {
    ...
    this.loadProvinces();
    this.omniUpdateDataJson({ SelectedAcess: 'inwib2c_fibreIAM'});
    this.handleIAMAccessChange() ; 
    console.log('this.hasIAMAccess Callback13', this.hasiamaccess) ;
 ...
 async CheckEligiblite() {
  ...
  if (this.ND !== '' && this.ND != null) {
    ...
    input = `{"operateur":"iam", "ND":"${this.Login}", "IdClient":"${this.IdClient}" , "RIO":"${this.RIO}"}`; // Y_MH MGEN3790 FTTH-Transfert à 3
  } 
  ...
  try {
    ...
    if (!response.error) {
      ...
      if (!this.isNDLoginEligible) {
         if (Operator == 'INWI') { //Y_MH MGEN3790 FTTH-Transfert à 3 begin
  ...
  if (this.isServiceAccIdCreated) {
    this.hasAccessOrangeVulaMig = "Oui";
    console.log("hasAccessOrangeVulaMig",this.hasAccessOrangeVulaMig);
    this.NextToOrange = false ; 
    this.omniUpdateDataJson({
    SelectedAcess: 'inwib2c_fibreIAM',
    hasAccessOrangeVulaMig: this.hasAccessOrangeVulaMig
    });
    this.omniSaveState({ hasAccessOrangeVulaMig: this.hasAccessOrangeVulaMig }, true);
    this.showMessage('Succès', 'Le logement est éligible. Merci de renseigner l’adresse.', 'success');
  } else {
    //04/07/2025 CH-Y MGEN3747 FTTH_VULA éligibilité exigences ANRT begin
    this.hasAccessOrangeVulaMig = "Non";
    console.log("hasAccessOrangeVulaMig",this.hasAccessOrangeVulaMig);
    this.NextToOrange = true ; 
    this.omniUpdateDataJson({
    SelectedAcess: 'inwib2c_fibreOrange',
    hasAccessOrangeVulaMig: this.hasAccessOrangeVulaMig
    });
    this.omniSaveState({ hasAccessOrangeVulaMig: this.hasAccessOrangeVulaMig }, true);
    this.showMessage('Erreur', 'Non éligible à la FTTH, vous pouvez passer à l’éligibilité Orange', 'error');
    //04/07/2025 CH-Y MGEN3747 FTTH_VULA éligibilité exigences ANRT End
  }
}
...
generateInputOS() {
  ...
  redirectEligibilityOrange() {
     
    // this.omniUpdateDataJson({ SelectedAcess: 'inwib2c_fibreOrange' })
      console.log('SelectedAcess gotonextStep 111', this.omniJsonData.SelectedAcess); 
      this.omniNextStep();
