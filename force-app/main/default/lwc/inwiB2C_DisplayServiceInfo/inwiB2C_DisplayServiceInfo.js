...
import template from './inwiB2C_DisplayServiceInfo.html';

export default class inwiB2C_DisplayServiceInfo extends OmniscriptBaseMixin(LightningElement) {
        ...
        this.__records =
          ...
          value.map((row, index) => {
            ...
            if(row.ProfilUser === "Inwi POS"){
                let account_link = `/PortailPDVPhase2/s/account/${row.Id}/view`;
                return { ...row, account_link, index: index + 1 };

            }
            if(row.ProfilUser === "BackOffice Distributeur D2D"){
                let account_link = `/BackOfficeDistributeurD2D/s/account/${row.Id}/view`;
                return { ...row, account_link, index: index + 1 };

            }
            let account_link = `/lightning/r/Account/${row.Id}/view`;
            return { ...row, account_link, index: index + 1 };
    ...
    @track columns =[
        {
            ...
            {fieldName: 'CreationDate', label: 'Date de création',type :"Date" , editable: false
            },
            {fieldName: 'NumDesignation', label: 'N° désignation',type :"Date" , editable: false
            },
            {fieldName: 'EligibilityFTTHADSL', label: 'Eligibilité FTTH/ADSL',type :"text" , editable: false
            },
            {fieldName: 'adress', label: 'Adresse du site',type :"text" , editable: false
        },
         ];
