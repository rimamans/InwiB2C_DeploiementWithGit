...
export default class Inwib2c_EligibilityFTTHTypeAcessOrangeForMigration extends OmniscriptBaseMixin(
  NavigationMixin(LightningElement)
) {
  vfRoot = "https://inwi-b2c--devevol--c.sandbox.vf.force.com";

  @track isSearchDone = false;
  @track hasOrangeAccess = "";
  @track ND = "";
  ...
  handleOrangeAccessChange(event) {
...
// apppel a la vip qui va retourner l'elegibilité begin
 ...
 this._actionUtilClass = new OmniscriptActionCommonUtil();
      ...
      // Vérification des champs nécessaires
          ...
          input = '{"operateur":"oma", "ND": "' + this.ND + '"}';
        ...
        .then(response => {
            if (!response.error) {
                ...
                const oiOperator = response.result?.IPResult?.oiOperator;
                 this.isNDEligible = (status === "OK" && oiOperator === "Orange");
                ...
                this.inputOs = '{"InOperatorOI":"ORANGE" ,"InNro":"' + this.nro +'" , "InTypePartage":"ACTIF" , "InNumeroDesignation":"' + this.ND +'", "InStatutLigne":"' + this.statutLigne +'", "InTypeAcess":"' + this.TypeAcess +'"}';
